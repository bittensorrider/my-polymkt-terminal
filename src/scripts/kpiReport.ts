...
async function main(): Promise<void> {
  ...
  console.log(
    `Both-fill rate: ${pct(fillRate)} (${both.length}/${records.length})`,
  );
  console.log(
    `One-sided / cut-loss exits: ${records.filter((r) => r.oneSided).length}`,
  );
  console.log(`Ghost-fill flags: ${records.filter((r) => r.ghostFill).length}`);
  console.log(
    `Total P&L: ${records.reduce((s, r) => s + r.pnl, 0).toFixed(4)} USDC`,
  );
  ...
  for (const minute of [...buckets.keys()].sort((a, b) => a - b)) {
    ...
    const rsBoth = rs.filter((r) => r.bothFilled);
    console.log(
      `  minute ${minute}: ${rs.length} cycles, fillRate=${pct(rsBoth.length / rs.length)}`,
    );
  }
  console.log("");
}
