...
export class FillWatcher {
  ...
  start(): void {
    if (config.dryRun) {
      logger.info(
        "[DRY_RUN] fill watcher disabled — strategy relies on simulated/on-chain polling instead.",
      );
      return;
    }
    this.stopped = false;
  ...
  private connect(): void {
    ...
    ws.on("message", (raw: RawData) => {
      try {
        ...
        const tokenId = (payload.asset ?? payload.tokenId) as
          | string
          | undefined;
        ...
        if (
          config.proxyWallet &&
          proxyWallet &&
          proxyWallet.toLowerCase() !== config.proxyWallet.toLowerCase()
        ) {
          return;
        }
