...
export function getPolygonProvider(): ethers.providers.JsonRpcProvider {
  if (!provider) {
    provider = new ethers.providers.JsonRpcProvider(config.polygonRpcUrl, CHAIN_ID);
  }
  return provider;
}
...
export async function initClient(): Promise<ClobClient> {
  ...
  client = new ClobClient(HOSTS.clob, Chain.POLYGON, wallet, creds, sigType, config.proxyWallet);
  logger.success(`CLOB client ready (signatureType=${sigType}, funder=${config.proxyWallet}).`);
  return client;
}
...
export async function getUsdcBalance(address: string): Promise<bigint> {
  const erc20 = new ethers.Contract(CONTRACTS.usdc, ERC20_ABI, getPolygonProvider());
...
export async function placeMakerBuy(
  ...
  if (config.dryRun) {
    ...
    logger.trade(`[DRY_RUN] would place GTC BUY ${shares} @ ${price} on token …${tokenId.slice(-8)}`);
  ...
  const orderId: string | undefined = resp?.orderID ?? resp?.orderId ?? resp?.id;
  if (!orderId) {
    throw new Error(`createAndPostOrder returned no order id: ${JSON.stringify(resp)}`);
  }
...
export async function marketSell(tokenId: string, shares: number): Promise<PlaceOrderResult> {
  if (config.dryRun) {
    const orderId = `SIM-SELL-${tokenId.slice(-8)}-${Date.now()}`;
    logger.trade(`[DRY_RUN] would market-SELL ${shares} of token …${tokenId.slice(-8)}`);
    return { orderId, dryRun: true };
  }
  ...
  const orderId: string | undefined = resp?.orderID ?? resp?.orderId ?? resp?.id;
  logger.trade(`Market-sold ${shares} of token …${tokenId.slice(-8)} -> order ${orderId ?? '(no id returned)'}`);
...
export async function getOrderStatus(orderId: string): Promise<OpenOrder | null> {
