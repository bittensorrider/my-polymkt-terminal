...
function printStatus(): void {
  ...
  logger.info(
    `status: dryRun=${config.dryRun} running=[${running}] queued=${pendingByAsset.size}`,
  );
}

async function maybeEnterCurrentMarkets(
  ...
  for (const market of current) {
    try {
      ...
      if (worst > config.currentMarketMaxOdds) {
        logger.info(
          `[${market.asset}] skipping in-progress ${market.slug} — odds already too lopsided (${worst.toFixed(3)}).`,
        );
        continue;
      }
    } catch (err) {
      logger.warn(
        `[${market.asset}] could not check in-progress odds for ${market.slug}: ${errMsg(err)} — skipping.`,
      );
      continue;
    }
